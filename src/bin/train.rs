use text_io::read;
use whister::{fortify::{QLearner, GameState}, game::Game, card::Card};
...
fn main() {
    ...
    println!("Start from existing model? [modelname/n]");
    let from_model: String = read!();
    if from_model != "n" {
        learner = QLearner::new(Game::new());
        learner.import_from_model(from_model.clone(), false);
    } else {
    ...
    loop {
        ...
        for _ in 0..gamesize {
            ...
            if slow {
                ...
                game.agent_plays_round(best_card_id, &mut learner);
            }
